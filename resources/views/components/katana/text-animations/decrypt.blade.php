...
<{{ $tag }} x-data="decryptedText({
    text: '{{ $text ?? 'Empty text' }}',
    characters: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()_+-={}[]',
    speed: {{ $speed }}
})" x-init="setTimeout(() => startDecrypt(), '{{ $delay }}')" {{ $attributes->twMerge('text-5xl') }}>
                ...
                startDecrypt() {
                    ...
                    this.scrambleInterval = setInterval(() => {
                        this.displayChars = processedText.split('').map((char, i) => {
                            if (this.revealedIndices.includes(i) || char === ' ' || !this.characters
                                .includes(char)) {
                                return char;
                            }
                            return this.getRandomChar();
                    ...
                    const revealNext = () => {
                        ...
                        while (revealIndex < processedText.length &&
                            (processedText[revealIndex] === ' ' || !this.characters.includes(processedText[
                                revealIndex]))) {
