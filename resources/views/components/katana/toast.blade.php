<div {{ $attributes->twMerge('fixed inset-0 px-2 pb-4 space-y-2 w-screen h-screen max-w-sm z-99999999') }}
    ...
    <template x-for="toast in toasts" :key="toast.id">
        <div :id="'katana-toast-' + toast.id" x-data popover="manual" class="flex overflow-hidden fixed top-0 flex-col items-start p-3.5 space-y-0.5 w-full max-w-sm text-sm text-white rounded-2xl opacity-100 duration-300 ease-out translate-y-0 starting:opacity-0 starting:-translate-y-full ending:-translate-y-full ending:opacity-0 backdrop-blur-xs group bg-black/60" role="alert">
            <!-- Progress Bar -->
