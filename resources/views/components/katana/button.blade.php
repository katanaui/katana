...
@php
    ...
    $defaultClasses = 'border-transparent no-underline bg-linear-to-b from-primary/90 via-primary/90 to-primary text-primary-foreground select-none focus-visible:ring-2 focus-visible:ring-primary/10 dark:focus-visible:ring-primary/20 focus-visible:ring-offset-2 focus-visible:ring-offset-background';
    switch ($variant ?? 'primary') {
        ...
        case 'secondary':
            ...
            break;
        case 'destructive':
            $typeClasses = 'border-transparent no-underline bg-red-600 focus-visible:ring-2 focus-visible:ring-red-100 dark:focus-visible:ring-red-900 focus-visible:ring-offset-2 focus-visible:ring-offset-background text-white' . $topHighlight;
            break;
        case 'outline':
...
<{!! $typeAttr !!} {{ $attributes->twMerge($sizeClasses . ' ' . $typeClasses . ' cursor-pointer border inline-flex items-center justify-center items-center font-medium focus-visible:outline-none') }}>
    ...
    @endif
    @if ($loader ?? false)
        <span class="flex absolute justify-center items-center w-full h-full" wire:loading.flex wire:target="{{ $wireTarget }}"><svg xmlns="http://www.w3.org/2000/svg" class="{{ $loaderClasses }} animate-spin" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
        </span>
    @endif
    <span @class(['flex items-center', 'opacity-0' => $loading]) @if ($loader ?? false) wire:loading.class="opacity-0" wire:target="{{ $wireTarget }}" @endif>{{ $slot }}</span>
</{{ $typeClose }}>
