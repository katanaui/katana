...
<section
    class="w-full h-full bg-background flex flex-col text-sm select-none"
    ...
    <div class="flex items-center px-5 pt-3 shrink-0">
        ...
        <div class="flex-1 min-w-0">
            ...
            @elseif($computedTitle)
                <h3 class="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground/60 truncate pl-0.5">{{ $computedTitle }}</h3>
            @endif
        </div>
        ...
        <div class="flex items-center gap-0.5 shrink-0">
            <button
                ...
                title="New File"
                :disabled="dtCreating"
                :class="dtCreating ? 'opacity-30 cursor-not-allowed' : 'hover:bg-muted hover:text-accent-foreground'"
                class="p-1.5 rounded-md text-muted-foreground transition-colors"
