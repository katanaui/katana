...
<{!! $containerAttribute !!} x-data="{ checked: {{ $checked ? 'true' : 'false' }} }" 
    ...
    @if(!$hideCheckbox)
        <span @if(!$clickable && !$disabled) @click="$refs.checkbox.checked = !$refs.checkbox.checked; $refs.checkbox.dispatchEvent(new Event('change')); $refs.checkbox.focus()" @endif 
            {{ $attributes->twMergeFor('checkbox', 'w-5 h-5 border flex-shrink-0 border-foreground/20 bg-background rounded-md flex items-center text-primary-foreground justify-center transition group-data-checked:border-primary group-data-checked:bg-primary peer-disabled:opacity-60 peer-focus:ring-2 peer-focus:ring-primary/20') }}>
            @if($icon)
                <x-dynamic-component :component="$icon" {{ $attributes->twMergeFor('checkmark', 'w-3.5 h-3.5 stroke-current duration-150 ease group-data-checked:opacity-100 opacity-0 group-data-checked:scale-100 scale-50') }} />
            @else
                <svg {{ $attributes->twMergeFor('checkmark', 'w-3.5 h-3.5 stroke-current duration-150  ease group-data-checked:opacity-100 opacity-0 group-data-checked:scale-100 scale-50') }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="3"><path d="M20 6 9 17l-5-5"/></svg>
            @endif
        </span>
    @endif
    ...
    @if($label || $description)
        <span class="flex flex-col peer-disabled:opacity-60">
            @if($label)
                <{!! $innerLabelAttribute !!} {{ $attributes->twMergeFor('label', 'text-sm font-medium select-none text-foreground/90') }}>
                    {{ $label }}
                </{!! $innerLabelAttribute !!}>
            @endif
